const rocketRow = document.querySelector('#rocketRow');
  ...
  if(res.length > 0) {
    ...
    */
    for(var i = 0; i < res.length; i++) {
      rocketRow.innerHTML += 
        '<div class="col-lg-4 col-md-6 col-xs-12 rocket-main"><div class="rocket">' + 
        '<img class="rocket-image img-fluid" src="https://cdn.filestackcontent.com/AXnEUKHYSo2FvV8zpGziAz/resize=width:350,height:400/compress/' + res[i].flickr_images[0] + '" width="350" height="400" alt="The SpaceX ' + res[i].rocket_name + ' rocket just before/after launch. The image is in black and white."/>' + 
        '<div class="rocket-info"><p class="rocket-country">' + res[i].country + '</p><h1 class="rocket-text">' + res[i].rocket_name + '</h1></div>' +
        '<div class="rocket-overlay"><a href="rocket-specific.html?id=' + res[i].rocket_id + '" class="btn rocket-button">READ MORE <i class="fas fa-chevron-right"></i></a></div></div></div>'
    }
  } else {
